...
def FlagsForCompilationDatabase(root, filename):
    logging.info("Trying to find flags for " + filename + " in compilation database, root = " + root)
    if True:
    # try:
        ...
        for folder in PathsToAllParentFolders(filename):
            ...
            for BUILD_DIRECTORY in BUILD_DIRECTORIES:
                path = folder + "/" + BUILD_DIRECTORY + "/compile_commands.json"
                logging.info(path)
                if os.path.isfile(path):
                    compilation_db_path = path
                    break
    ...
    #     logging.info("Expection while trying to find flags for " + filename + " in compilation database: " + str(e))
    #     return None
